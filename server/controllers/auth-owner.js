...
function parseToken(token) {
  try {
    // console.log(token)
    const tokenObject = JSON.parse(token.substring(7)); // Removes "Bearer " prefix and parses the remaining JSON string
    
    return jwt.verify(tokenObject.token,process.env.JWT_SECRET);
  } catch (err) {
    console.log(err)
    return false;
  }
