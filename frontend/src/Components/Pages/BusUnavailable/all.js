...
const AllBusUnavailable = () => {
  ...
  const fetchUnAvailableBuses = async () => {
    const response = await getAllUnavailableBuses().catch((err) => {
      setError(err.response.data.error);
      setIsLoading(false);
    });
...
export default AllBusUnavailable;
